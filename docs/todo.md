# TODO 清单（本地 PC：每日新闻采集/分析/展示）

## 阶段 1：每日新闻闭环（MVP+，优先级最高）

- （已完成）今日新闻列表接口 `GET /news/today`（默认当日、支持 market/tickers/query/排序）
- （已完成）手动触发刷新 `POST /admin/refresh`（用于手工采集）
- （已完成）日报摘要接口 `POST /daily/summary`（今日新闻 -> Qwen 摘要）
- 关注清单与过滤逻辑（用户关注市场/标的/关键词，默认应用到今日新闻）
- `/analysis` 输出模板固化（结论/影响/风险/关注点，带 [n] 证据引用）
- 分析结果缓存（按问题+来源 hash），降低重复 token 消耗
